...
export default function PromotionTestClient({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    const res = await submitPromotionTest(answers);
    setSubmitting(false);
    ...
    return (
      <section className="dash-card dash-card--full">
        ...
        <p className="content-block__empty">
          <i className="fa-solid fa-lock" aria-hidden="true" /> You can only take this test once every 7 days. You're
          eligible again on <strong>{nextEligibleLabel}</strong>.
        </p>
        {lastScore && (
          <p className="content-block__empty">
